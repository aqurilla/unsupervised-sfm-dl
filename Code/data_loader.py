...
class DataLoader(object):
    ...
    def load_train_batch(self):
        ...
        # For further changes in data augmentation strategies
        # change within data_augmentation()
    ...
    def data_augmentation(self, im, intrinsics, out_h, out_w):
        ...
        def color_aug(im):
            batch_size, in_h, in_w, n_img = im.get_shape().as_list()
            n_img = int(n_img/3)
            ...
            for i in range(batch_size):
                ...
                for j in range(n_img):
                    im1 = tf.image.random_hue(im[i,:,:,3*j:3*(j+1)], 0.075)
                    im1 = tf.image.random_saturation(im1, 0.0, 1.1)
                    im1 = tf.image.random_brightness(im1, 0.1)
                    im1 = tf.image.random_contrast(im1, 0.0, 0.15)
                    stck_im.append(im1)
                im2 = tf.concat(stck_im, 2)
                aug_im.append(tf.expand_dims(im2, 0))
            im = tf.concat(aug_im, 0)
