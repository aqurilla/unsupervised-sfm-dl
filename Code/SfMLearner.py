...
class SfMLearner(object):
    ...
    def build_train_graph(self):
        ...
        with tf.name_scope("data_loading"):
            tgt_image, src_image_stack, intrinsics = loader.load_train_batch() # loading and augmentation
            tgt_image = self.preprocess_image(tgt_image) # converting to float
        ...
        with tf.name_scope("compute_loss"):
            ...
            exp_mask_stack_all = []
            for s in range(opt.num_scales): # total loss is averaged over all scales considered
                # CHANGED
                scale_pixel_loss = 0

                # In current version of the program, authors have not used explainability regularization
                ...
                if opt.smooth_weight > 0: # weight for smoothness (lambda_s)
                    smooth_loss += opt.smooth_weight/(2**s) * \
                        self.compute_smooth_loss(pred_disp[s]) # smoothness loss
                        # pred_disp[s] selects the scale out of 4 generated DispNet scales
                ...
                #     else:
                #         pixel_loss += tf.reduce_mean(curr_proj_error) # pixel loss
                ...
                for i in range(opt.num_source):
                    ...
                    # Implement masking
                    curr_src_image = curr_src_image_stack[:,:,:,3*i:3*(i+1)]

                    # if tf.reduce_mean(tf.abs(curr_proj_image - curr_tgt_image)) < \
                    #     tf.reduce_mean(tf.abs(curr_src_image - curr_tgt_image)):
                    #     curr_proj_error = tf.abs(curr_proj_image - curr_tgt_image)
                    # else:
                    #     curr_proj_error = tf.abs(curr_src_image - curr_tgt_image)

                    # Pixel-masking (mask out pixels where unwarped image has lower reprojection error)
                    pmask = tf.greater(tf.abs(curr_src_image - curr_tgt_image), tf.abs(curr_proj_image - curr_tgt_image))
                    pmask_f = tf.cast(pmask, dtype=tf.float32)
                    curr_proj_error = tf.multiply(pmask_f, curr_proj_error)

                    # Implement the minimum reprojection error
                    # Concat the error over all source images
                    if i == 0:
                        min_error_stack = tf.expand_dims(curr_proj_error, axis=0)
                    else:
                        min_error_stack = tf.concat([min_error_stack,
                                                      tf.expand_dims(curr_proj_error, axis=0)], axis=0)
                ...
                # Calculate pixel-wise minimum photometric error over all source image at scale s
                scale_pixel_loss += tf.reduce_min(min_error_stack, axis=0)
            ...
            # Average pixel-loss minimumm error over pixel, scale and batch
            pixel_loss += tf.reduce_mean(scale_pixel_loss)
            total_loss = pixel_loss + smooth_loss + exp_loss # total loss
            # exp loss unused
    ...
    def train(self, opt):
        ...
        opt.num_scales = 4 # number of scales changed to 4
        self.opt = opt
